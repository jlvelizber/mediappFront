...
export default function Patients() {
  const TITLE_PAGE = messages.patient.pages.listTitle;
  ...
  return (
    <>
      <DeleteConfirmation entityName={messages.patient.entityName} isOpen={flowDeletePatient.isOpenDeleteConfirmation} onClose={handleCloseDeleteConfirmation} onConfirm={handleConfirmDelete} />
      <DashboardLayout>
        <PageWrapper>
          <div className="container mx-auto p-4">
            <div className="flex justify-between items-center mb-4">
              <h1 className="text-2xl font-bold mb-4">{messages.patient.pages.listTitle}</h1>

              {/* 🔹 Botón SIEMPRE visible */}
              <Link href={`${patientsBaseRoute}/create`}>
                <button className="btn-primary">
                  <PlusIcon className="w-5 h-5 mr-2" />
                  {messages.patient.pages.addButton}
                </button>
              </Link>
            </div>
            ...
            {!hasDataInFirstFetch ? (
              ...
              <>
                <input
                  type="text"
                  placeholder={messages.patient.pages.searchPlaceholder}
                  value={search}
