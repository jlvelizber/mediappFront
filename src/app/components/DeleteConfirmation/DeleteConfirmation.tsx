import { formatMessage, messages } from "@/app/config/messages";
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
            <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md mx-4">
                <h2 className="text-lg font-semibold text-gray-900">
                    {formatMessage(messages.deleteConfirmation.title, { entity: entityName })}
                </h2>
                <p className="text-gray-600 mt-2 text-sm">
                    {messages.deleteConfirmation.messageBeforeEntity}
                    <strong>{entityName}</strong>
                    {messages.deleteConfirmation.messageAfterEntity}
                </p>
